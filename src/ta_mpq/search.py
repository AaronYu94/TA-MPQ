...
def resolve_group_value_scores(
    ...
    if layer_stats is not None and target_grouping and target_grouping != source_grouping:
        ...
        aggregated_score = aggregate_group_score_overrides(
            layer_stats=layer_stats,
            score_overrides={
                group_name: float(profile.get("score", 0.0))
                for group_name, profile in source_scores.items()
            },
            target_grouping=target_grouping,
            source_grouping=source_grouping,
        )
        aggregated_uplift_8 = aggregate_group_score_overrides(
            layer_stats=layer_stats,
            score_overrides={
                group_name: float(profile.get("uplift_8_over_4", profile.get("score", 0.0)))
                for group_name, profile in source_scores.items()
            },
            target_grouping=target_grouping,
            source_grouping=source_grouping,
        )
        aggregated_uplift_16 = aggregate_group_score_overrides(
            layer_stats=layer_stats,
            score_overrides={
                group_name: float(profile.get("uplift_16_over_8", 0.0))
                for group_name, profile in source_scores.items()
            },
            target_grouping=target_grouping,
            source_grouping=source_grouping,
        ...
        return {
            group.name: _group_value_profile(
                {
                    "score": float(aggregated_score.get(group.name, 0.0)),
                    "uplift_8_over_4": float(aggregated_uplift_8.get(group.name, 0.0)),
                    "uplift_16_over_8": float(aggregated_uplift_16.get(group.name, 0.0)),
                }
            )
            for group in groups
        }
