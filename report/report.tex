...
\usepackage{xcolor}
\usepackage{url}
\usepackage[nohyperref]{../mlsys2025style/mlsys2025}
...
\begin{abstract}
Deploying large language models under strict memory budgets requires more than applying a uniform low-bit recipe to every layer. This paper presents \method{}, a task-aware mixed-precision quantization pipeline that allocates \inttwo{}/\intfour{}/\inteight{} precision under the same raw weight footprint as a uniform \intfour{} model. The method profiles group-level task sensitivity, builds a deterministic greedy path of unique budget-feasible policies, and selects coarse and refined candidates from real path snapshots rather than requested percentage targets. This draft follows the required paper format and reserves the Results section for the final quantitative tables, plots, and ablations. The current contribution is therefore the problem formulation, algorithmic design, implementation-grounded protocol, and explicitly stated limitations.
\end{abstract}
]
...
The final evaluation suite is intended to cover both recognition-style and generation-style tasks. HumanEval measures ``functional correctness for synthesizing programs from docstrings'' \cite{chen2021codex}. EvalPlus augments HumanEval with stronger tests for generated code \cite{liu2023evalplus}. BigCodeBench targets code generation with ``diverse function calls as tools'' and complex instructions \cite{zhuo2025bigcodebench}. MATH introduces ``12,500 challenging competition mathematics problems'' and is useful for reasoning-heavy evaluation \cite{hendrycks2021math}. The Results section is reserved for the final numbers, but these benchmarks define the planned evidence base.
...
The final results should compare the selected mixed policy against BF16, uniform \inteight{}, and uniform \intfour{} baselines under the same task protocol. For classification-style and math tasks, the table should report accuracy, correct count, token-limit hit count, average completion tokens, median completion tokens, p95 completion tokens, average latency, median latency, and p95 latency. For code-generation tasks, the table should report pass@1, execution status, capped-generation counts, and completion-token statistics.
...
This section is intentionally left as a placeholder for the final report numbers. It should not be replaced by informal run notes; it should contain final, reproducible tables and figures tied to exact artifact paths, benchmark splits, decoding settings, and policy identifiers.

\begin{table*}[t]
\caption{Placeholder for final accuracy and token-usage comparison. Fill this table only after freezing official artifacts and baselines.}
\label{tab:results-placeholder}
\centering
\begin{tabular}{lcccccc}
\toprule
Model & Split & Metric & Score & Avg tokens & P95 tokens & Token-wall hits \\
\midrule
BF16 & TBD & accuracy/pass@1 & TBD & TBD & TBD & TBD \\
Uniform \inteight{} & TBD & accuracy/pass@1 & TBD & TBD & TBD & TBD \\
Uniform \intfour{} & TBD & accuracy/pass@1 & TBD & TBD & TBD & TBD \\
\method{} mixed & TBD & accuracy/pass@1 & TBD & TBD & TBD & TBD \\
\bottomrule
\end{tabular}
\end{table*}

The final version should also include an ablation table for the design choices that are central to the claim: task sensitivity profiling, exact-budget demotion repair, path-based coarse selection, and local refinement. If any benchmark shows no gain over uniform \intfour{}, that failure case should be reported directly rather than hidden.
...
The current benchmark sizes for coarse and refine selection are modest. This makes tie handling and disjoint question sets important, but it also means small accuracy differences should not be overclaimed. The final Results section should report uncertainty or at minimum discuss sampling sensitivity.
...
This paper frames task-aware mixed-precision quantization as exact-budget policy construction. The key idea is to spend \inteight{} precision where a task-sensitive profiler predicts high value, pay for it with low-risk \inttwo{} demotions, and search over real path snapshots rather than requested percentages. The draft now defines the problem, method, experimental setup, placeholder result structure, and limitations in a stricter paper format. The remaining step is to fill the Results section with final frozen artifacts and to align every empirical claim in the Abstract and Introduction with those numbers.
